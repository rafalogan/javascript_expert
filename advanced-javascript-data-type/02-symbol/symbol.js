...
// É difeicil de pegar, mas não é secreto!
...
class MyDate {
	...
	[Symbol.toPrimitive](correctionTypes) {
		if (correctionTypes !== 'string') throw new TypeError();

		const items = this[kItems].map(item => new Intl.DateTimeFormat('pt-BR', {month: 'long', day: '2-digit', year: 'numeric'})
			.format(item));
...
// Conversão explicita para chamar o toPrimitive
assert.deepStrictEqual(String(myDate), '01 de abril de 2020 e 02 de março de 2018');
assert.deepStrictEqual([...myDate], expectedDates);

// ;(async () => {
// 	for await (const item of myDate) console.log('asyncIterator', item);
// })()
